...
import { MapPin, Calendar, ChevronDown, ChevronUp, Construction, PartyPopper, Stethoscope, DollarSign, User, Users } from 'lucide-react';
...
export default function LeadTable({ leads, type }: LeadTableProps) {
  ...
  const getPurchaseButton = (lead: Lead) => {
    ...
    if (lead.purchasedBy.includes(user.id)) {
      return (
    ...
    return (
      ...
      >
        ...
        <span className="ml-2 text-xs bg-blue-500 px-2 py-0.5 rounded-full">
          {3 - lead.purchasedBy.length} left
        </span>
      </button>
    );
  };
